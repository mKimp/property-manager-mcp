...
exports.registerRentTools = registerRentTools;
...
function registerRentTools(server) {
    // ADD RENT RECORD
    server.registerTool("rent_add_record", {
...
Returns: The newly added rent record.`,
        inputSchema: index_js_1.AddRentRecordInputShape,
        ...
        if (exists) {
            return {
                content: [{ type: "text", text: `Error: A rent record for ${MONTH_NAMES[month]} ${year} already exists. Use rent_update_record to modify it.` }],
            };
        }
        const record = { year, month, amount, paid, paidDate, notes };
        ...
        return {
            content: [{ type: "text", text: `Rent record for ${MONTH_NAMES[month]} ${year} added to '${property.propertyName}'.\n\n${JSON.stringify(record, null, 2)}` }],
        };
    });
    // UPDATE RENT RECORD
    server.registerTool("rent_update_record", {
...
Returns: The updated rent record.`,
        inputSchema: index_js_1.UpdateRentRecordInputShape,
        ...
        if (!record) {
            return {
                content: [{ type: "text", text: `Error: No rent record found for ${MONTH_NAMES[month]} ${year} on '${property.propertyName}'.` }],
            };
        }
        if (paid !== undefined)
            record.paid = paid;
        ...
        (0, storage_js_1.saveProperty)(property);
        return {
            content: [{ type: "text", text: `Rent record for ${MONTH_NAMES[month]} ${year} updated on '${property.propertyName}'.\n\n${JSON.stringify(record, null, 2)}` }],
        };
    });
    // GET RENT RECORDS BY YEAR
    server.registerTool("rent_list_by_year", {
...
Returns: Monthly rent records with totalCollected, totalDue, unpaidMonths, and collectionRate.`,
        inputSchema: index_js_1.GetRentByYearInputShape,
    ...
    server.registerTool("rent_list_all", {
...
Returns: All rent records grouped by year with annual collected/due totals.`,
        ...
        for (const record of property.rentRecords) {
            ...
            if (record.paid)
                byYear[record.year].totalCollected += record.amount;
        }
