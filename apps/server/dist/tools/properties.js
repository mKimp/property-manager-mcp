...
Returns: Array of property summaries with id, propertyName, address, rent, currency, tenants, leaseStart, leaseEnd, propertyManager, notes.`,
    ...
    // GET PROPERTY BY ID
    server.registerTool("property_get", {
...
Returns: Full property object with repairs[] and rentRecords[].`,
    ...
    // ADD PROPERTY
    server.registerTool("property_add", {
        title: "Add Property",
...
Returns: The newly created property with its generated ID.`,
        inputSchema: index_js_1.AddPropertyInputShape,
    ...
    // UPDATE PROPERTY
    server.registerTool("property_update", {
...
Returns: The updated property object.`,
        ...
        if (mortgage !== undefined)
            property.mortgage = { ...property.mortgage, ...mortgage, monthlyPayment: mortgage.monthlyPayment ?? property.mortgage?.monthlyPayment ?? 0 };
    ...
    // SEARCH PROPERTIES
    server.registerTool("property_search", {
...
If multiple matches are found, present them to the user and ask which one they mean before proceeding.`,
        ...
        const matches = properties.filter((p) => {
            if (city && !p.address.city.toLowerCase().includes(city.toLowerCase()))
                return false;
            if (state && !p.address.state.toLowerCase().includes(state.toLowerCase()))
                return false;
            if (propertyName && !p.propertyName.toLowerCase().includes(propertyName.toLowerCase()))
                return false;
            if (street && !p.address.street.toLowerCase().includes(street.toLowerCase()))
                return false;
            if (tenantName && !p.tenants.some((t) => t.name.toLowerCase().includes(tenantName.toLowerCase())))
                return false;
            return true;
        });
        if (matches.length === 0) {
            return { content: [{ type: "text", text: "No properties found matching the search criteria." }] };
        }
    ...
    // DELETE PROPERTY
    server.registerTool("property_delete", {
