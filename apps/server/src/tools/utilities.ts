...
Returns: The newly added utilities record.`,
      inputSchema: AddUtilitiesRecordInputShape,
    ...
    async ({ propertyId, year: yearInput, month: monthInput, utilities, paid, paidDate, notes }) => {
      ...
      if (exists) {
        return {
          content: [{ type: "text" as const, text: `Error: An utilities record for ${MONTH_NAMES[month]} ${year} already exists. Use utilities_update_record to modify it.` }],
        };
      }
...
Returns: The updated utilities record.`,
      inputSchema: UpdateUtilitiesRecordInputShape,
    ...
    async ({ propertyId, year: yearInput, month: monthInput, paid, paidDate, utilities, notes }) => {
      ...
      if (!record) {
        return {
          content: [{ type: "text" as const, text: `Error: No utilities record found for ${MONTH_NAMES[month]} ${year} on '${property.propertyName}'.` }],
        };
      }
  ...
  server.registerTool(
    "utilities_list_by_year",
    {
      ...
      inputSchema: GetUtilitiesByYearInputShape,
    ...
    async ({ propertyId, year: yearInput }) => {
      ...
      const sumUtilities = (u: Utilities) =>
  ...
  server.registerTool(
    ...
    async ({ propertyId }) => {
      ...
      const sortedRecords = [...property.utilitiesRecords].sort((a, b) =>
